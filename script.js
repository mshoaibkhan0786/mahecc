document.addEventListener('DOMContentLoaded', () => {
    ...
    const highlightCurrentClass = () => {
        ...
        if (!timetable) return; // Only run on timetable page
        ...
        const cells = dayRow.querySelectorAll('td[data-time]');
        cells.forEach(cell => {
            const timeRange = cell.dataset.time.split('-');
            const startTime = parseInt(timeRange[0].split(':')[0]) + (parseInt(timeRange[0].split(':')[1] || 0) / 60);
            const endTime = parseInt(timeRange[1].split(':')[0]) + (parseInt(timeRange[1].split(':')[1] || 0) / 60);
    ...
    const renderGallery = async () => {
        ...
        const placeholder = document.querySelector('#coming-soon-placeholder');
        if (!container) return;

        if (!supabaseClient) {
            placeholder.innerHTML = `<h2 class="text-xl font-bold text-red-500">Error</h2><p>Supabase client not initialized.</p>`;
            return;
        }
        ...
        try {
            ...
            // Filter out the placeholder file that Supabase creates in empty buckets
            const images = data.filter(file => !file.name.startsWith('.emptyFolderPlaceholder'));

            if (images.length > 0) {
                 placeholder.classList.add('hidden'); // Hide the "Coming Soon" message
                 container.innerHTML = ''; // Clear the container for images
                 ...
                 images.forEach(file => {
                    ...
                    const imgElement = `
                        <div class="bg-[var(--card-bg)] rounded-lg shadow-md overflow-hidden border border-[var(--border-color)]">
                            <img src="${publicUrl}" alt="${file.name}" class="w-full h-48 object-cover hover:scale-105 transition-transform duration-300">
                        </div>
                    `;
                    container.innerHTML += imgElement;
                 });
            }
            // If images.length is 0, do nothing, the "Coming Soon" placeholder will remain visible.

        } catch (error) {
            console.error('Error fetching gallery images:', error);
            placeholder.innerHTML = `<h2 class="text-xl font-bold text-red-500">Error</h2><p>Could not load gallery. Check storage bucket permissions.</p>`;
        }
    };
    ...
    highlightCurrentClass(); // For timetable page
    renderGallery();         // For gallery page
    
    // Set an interval to re-check the time every minute for highlighting
