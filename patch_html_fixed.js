...
<div class="relative hidden md:block" id="profile-dropdown-container">
    ...
    <div id="profile-dropdown" class="profile-dropdown-menu hidden right-0 absolute mt-2 w-72 rounded-md shadow-lg bg-[var(--card-bg)] ring-1 ring-black ring-opacity-5 divide-y divide-[var(--border-color)]">
        <div class="px-4 py-3 pb-4">
            ...
            <p class="text-xs text-[var(--accent-color)] font-medium truncate mt-2 bg-[var(--bg-color)] inline-block px-2 py-1 rounded" id="profile-reg">Reg: Not set</p>
        </div>
        <div class="px-4 py-3 hidden bg-[var(--bg-color)] rounded-b-md" id="edit-profile-form">
            ...
            <input type="text" id="edit-reg-input" class="mt-1 block w-full rounded-md border-[var(--border-color)] bg-[var(--card-bg)] shadow-sm focus:border-[var(--accent-color)] focus:ring-[var(--accent-color)] text-[var(--text-color)] sm:text-sm px-2 py-1">
...
<div class="px-5 pt-4 pb-4 border-b border-[var(--border-color)] bg-[var(--card-bg)]" id="mobile-profile-section">
    <div class="flex items-center">
        <div class="flex-shrink-0 h-10 w-10 rounded-full bg-[var(--accent-color)] flex items-center justify-center text-white font-bold text-lg shadow-sm" id="mobile-profile-initial">?</div>
...
function processDirectory(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            if (!fullPath.includes('login')) { // patch all other index.html
                ...
                // Fix missing theme mobile logout container by recreating the whole toggle section safely
                // Some pages might have a broken logout since we appended randomly in the last script
                // Cheerio makes this easier to find and clean
                const themeToggleContainer = $('.border-t.border-\\[var\\(--border-color\\)\\].px-2.py-3.space-y-2');
                if (themeToggleContainer.length) {
                    // if it already exists, replace to make sure it's clean
                }
